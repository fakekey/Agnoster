...
#ifndef AGNOSTER_SDK_H
#define AGNOSTER_SDK_H

#include <codecvt>
#include <locale>
#include <string>

typedef void (*ProcessEventFn)(struct UObject *caller, struct UFunction *func, void *params);

namespace Offsets {
    constexpr uint64_t GNames = 0xC52EB80;
    constexpr uint64_t GObjects = 0xC54E2B0;
    constexpr uint64_t GWorld = 0xC75A4E8;
    constexpr uint64_t EntryPoint = 0x448F904;
    constexpr uint64_t AndroidApp = 0xC091F40;
} // namespace Offsets
...
struct UObject {
    ...
    void ProcessEvent(struct UFunction *func, void *params);
...
struct TUObjectArray {
    ...
    std::vector<T> FindObjects(const char *name) {
        ...
        for (int i = 0; i < NumElements; ++i) {
            auto objectItem = GetObjectPtr(i);
            if (objectItem && objectItem->Object && objectItem->Object->GetFullName().find(name) != std::string::npos)
...
struct UStruct : UField {
    // Fields
    char pad_0x30[0x10]; // Offset: 0x38 | Size: 0x10
    struct UStruct *SuperField;
...
struct ULevelActorContainer : UObject {
    // Fields
    struct TArray<struct AActor *> Actors; // Offset: 0x28 | Size: 0x10
};
...
struct UGameplayStatics_GetAllActorsOfClass_Params {
  public:
    UObject *WorldContextObject; // 0x0000(0x0008)  (ConstParm, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash, NativeAccessSpecifierPublic)
    UClass *ActorClass;          // 0x0008(0x0008)  (Parm, ZeroConstructor, IsPlainOldData, NoDestructor, UObjectWrapper, HasGetValueTypeHash, NativeAccessSpecifierPublic)
    TArray<AActor *> OutActors;  // 0x0010(0x0010)  (Parm, OutParm, ZeroConstructor, NativeAccessSpecifierPublic)
};

// Object: Class Engine.GameplayStatics
// Inherited Bytes: 0x28 | Struct Size: 0x28
struct UGameplayStatics : UBlueprintFunctionLibrary {
    // Object: Function Engine.GameplayStatics.GetAllActorsOfClass
